...
/*
   ...
   - véletlenszerû stringet be kell állítani mt4ID, amit szintén tárol az
...
double LotVol(double lot,string symbol) {
  ...
  // Ha azonban itt nincs egyezõ pár akkor érintetlen marad
  ...
  if (StringLen(LotMapping)>2) {
      ...
      for (int i=0;i< LotMapString1Width;i++) {
         ...
         // ha a map pár elsõ eleme egyenlo a paraméterként adott lottal, akkor a második eleme szerint alakul a lot!
...
//  Print("Calculated lot size: ",lot);
  // Print ("A következõ lot méretet használom: "+lot);
...
// if not market order, compare open prices - later 
    ...
    if (!found) {
      ...
      // De elõször regisztráljuk az sqlite adatbázisunkban. hogy többet már ne használjuk fel
      if (!check_OrderID(OrdId[i]) ) { //ha van már ilyen orderId az adatbázisban, nem írjuk be!
